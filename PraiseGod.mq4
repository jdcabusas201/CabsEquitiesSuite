...
void ClosePositionsIfProfitAbove(double profitThreshold)
{
    ...
    if (overallProfit > profitThreshold || ClosePositionsIfEquityBelowPercentOfBalance(BalanceThreshold))
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--)
        {
            ...
            {
                if (OrderClose(OrderTicket(), OrderLots(), MarketInfo(OrderSymbol(), MODE_BID), 0, 0))
                {
...
void sendBuyOrder(bool initial)
{
      ...
      //if (order_type == ORDER_TYPE_SELL)
        ...
        if (currentHour > 12 && currentHour < 20 && initial == true) {
          OrderSend(Symbol(), OP_BUY, size, cost, 2, 0, 0, magic, 0, 0, clrNONE);  // ORDER SEND
         ...
         OrderSend(Symbol(), OP_BUY, size, cost, 2, 0, 0, magic, 0, 0, clrNONE);
        }
}
...
void sendSellOrder(bool initial)
{
      ...
      //if (order_type == ORDER_TYPE_SELL)
        ...
        if (currentHour > 12 && currentHour < 20 && initial == true) {
          OrderSend(Symbol(), OP_SELL, size, cost, 2, 0, 0, magic, 0, 0, clrNONE);  // ORDER SEND
          //last_profit_level = cost;    // Update the last profit level
         ...
         OrderSend(Symbol(), OP_SELL, size, cost, 2, 0, 0, magic, 0, 0, clrNONE);
        }
}
...

{
    ...
    //if(ordertype == POSITION_TYPE_SELL)
      ...
      if (currentPrice >= Sellthreshold10Pips && SAdj1 == false && currentPrice > upper && currentPrice > last_sell_bb_price)
    {
    ...
    if (currentPrice >= Sellthreshold20Pips && SAdj2 == false && currentPrice > upper && currentPrice > last_sell_bb_price)
    ...
    if (currentPrice >= Sellthreshold30Pips && SAdj3 == false && currentPrice > upper && currentPrice > last_sell_bb_price)
...

{
    ...
    //if(ordertype == POSITION_TYPE_SELL)
      ...
      if (currentPrice <= Buythreshold10Pips && BAdj1 == false && currentPrice < lower && currentPrice < last_buy_bb_price)
    {
    ...
    if (currentPrice <= Buythreshold20Pips && BAdj2 == false && currentPrice < lower && currentPrice < last_buy_bb_price)

    {
     ...
     if (currentPrice <= Buythreshold30Pips && BAdj3 == false && currentPrice < lower && currentPrice < last_buy_bb_price)
...
//OpenPositions();
   ...
   if((iClose(_Symbol, PERIOD_CURRENT,0) > upper) && (upper - middle) > 100 * _Point && sell_entry_made == false){
      Print("Sell");
      sendSellOrder(true);
   }else if((iClose(_Symbol, PERIOD_CURRENT,0) < lower) && (middle - lower) > 100 * _Point && buy_entry_made == false){
      Print("BUY");
      sendBuyOrder(true);
   }
